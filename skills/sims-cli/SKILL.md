...
sims proxy run <device> [--port N] [--har out.har] [--for 30s] [--all] [--quiet] [--ssid <wifi>] [--setup]
...
sims proxy clean                            # put back what a capture killed mid-run left behind
...
It starts by listing what it set up on the device, one row per step; a row marked `!` is something sims could not do from here, and the taps for it are numbered under the row (`--json` on `proxy ca --install` carries them as `todo`). On an iPhone sims serves the profile from the capture's own port and opens that URL in the phone's Safari through devicectl, so the user only taps Allow there; once the phone has fetched it, sims offers to open Settings on the phone (enter in a terminal, a prompt in the TUI), where the user taps Profile Downloaded and Install, then switches the certificate on under Certificate Trust Settings. When Safari cannot be opened (phone locked, tunnel down) the step says so and gives the URL to type. On an Android phone without root it is picking the certificate from Downloads inside Settings; Android 11+ refuses a CA certificate from anywhere else, including the installer intent. `sims proxy ca <phone> --install` serves a certificate-only profile the same way and holds until the phone fetches it. On an Android emulator whose image takes `adb root` sims writes the system store itself and nothing is left to tap; a `google_apis_playstore` image refuses root ("adbd cannot run as root in production builds") and takes the manual path, so prefer an image without the Play Store for captures. Relay those taps to the user as they are printed, and expect HTTPS rows to say `tunnel` until they are done.
It prints one line per exchange (`--json` prints a record per flow, headers and both bodies included: `requestBody` and `responseBody`, decoded from gzip/brotli, with `responseBodyEncoding: "base64"` when the body is not text, and `resource` naming the bucket a browser's network panel would file it under: `xhr`, `doc`, `js`, `css`, `img`, `font`, `media`, `ws` or `other`, from the response Content-Type, else the path's extension, else `xhr`) and restores the device and this
...
| proxy rows all say `tunnel` on an iPhone | The profile is installed but the certificate is not switched on: Settings > General > About > Certificate Trust Settings, then switch `sims proxy CA` on. |
| `a phone takes its proxy from the wifi network it is on, and this Mac is not on one` | Pass the phone's wifi name: `sims proxy run <phone> --ssid <name>` (the TUI asks for it). A phone's proxy profile attaches to a named network, and a wired Mac cannot see which one the phone is on. Or `sims proxy ca <phone> --install` and set the proxy on the phone by hand. |
| The phone installed the profile but no traffic shows | A profile from `proxy ca --install` carries the certificate only. Traffic needs a running capture (`proxy run` or `t` in the TUI), whose profile also carries the proxy; and the certificate has to be switched on under Certificate Trust Settings for HTTPS to open. |
...
| `devicectl [device profile install ...]: exit status 64` | An older build sent the profile through a devicectl command Xcode does not have. The current build serves the profile at a URL the phone opens in Safari; update sims. |
| After a phone capture the phone has no network on that wifi | Its profile still sends traffic to this Mac on the fixed port (9797 unless `--port` chose another), which is by design so the next capture needs nothing installed. `sims proxy standby <phone>` or an open TUI relays it untouched; to stop for good, remove the profile on the phone under Settings > General > VPN & Device Management. |
| A phone capture starts with "already on the phone" but records nothing | The phone lost or changed the profile (removed it, or is on another wifi). `sims proxy run <phone> --setup` sends it again; the record of what the phone was given lives under the cert directory in `phones/`. |
